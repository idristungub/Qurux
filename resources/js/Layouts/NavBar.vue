    ...
    <div class=" sticky top-0 left-0 flex justify-between items-center  bg-[#6B8F71] w-full h-[99px] px-6">
        ...
        <div class="flex items-center gap-10 text-white ">
            <div v-if="$page.props.auth.user" class="flex items-center gap-6" >
                <Link class="hidden lg:block" :href="route('quran-quest.home')">
                    Home
                </Link>
                <Link class="hidden lg:block" :href="route('achievements.create')">
                    Achievement
                </Link>
                <Link class="hidden lg:block" :href="route('leaderboard.create')">
                    Leaderboard
                </Link>
            ...
            <template v-else>
                ...
                <Link class="bg-[#AAD2BA] w-[82.83px] h-[42px] rounded-[10px] py-2 px-5 hidden lg:block " :href="route('login')">
                    Login
                </Link>
                <Link class="bg-[#AAD2BA] w-[82.83px] h-[42px] rounded-[10px] py-2 px-3 hidden lg:block" :href="route('register')" >
                    Register
                </Link>
            </template>
        ...
        <div v-if="isMenuOpen" class="lg:hidden absolute flex flex-col items-center bg-[#6B8F71] w-full top-[90px] right-0 ">
            ...
            <template v-else>
                ...
                <div class="flex gap-7">
                    <Link class="bg-[#AAD2BA] w-[82.83px] h-[42px] rounded-[10px] py-2 px-5" :href="route('login')" >
                        Login
                    </Link>
                    <Link class="bg-[#AAD2BA] w-[82.83px] h-[42px] rounded-[10px] py-2 px-3" :href="route('register')" >
                        Register
                    </Link>
                </div>
